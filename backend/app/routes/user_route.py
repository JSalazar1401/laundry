...
def login():
    ...
    return jsonify({"msg":"Algo salio mal intentar iniciar sesion, revise sus claves :3"}), 400

@user_bp.route("/logout/<int:user_id>", methods=["POST"])
def logout(user_id):
    logout_user(user_id)
    return jsonify({"msg":"Logout exitoso :3"}), 200
...
def update(user_id):
    ...
    return jsonify({"msg":"Algo salio mal al intentar actualizar al usuario >:C"}), 400
...
def change_status(user_id):
    ...
    return jsonify({"msg":"Algo salio mal al intentar actualizar el estado del usuario >:C"}), 400
...
def delete(id):
    ...
    return jsonify({"msg":"Usuario eliminado con exito :3"}), 200
